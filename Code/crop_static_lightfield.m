function crop_static_lightfield(cropped_size, overlap, static_path, save_path)
% clear
% cropped_size = 176;
% overlap = 0.5;
...
% [file_name,  path] = uigetfile({ '*.tif; *.png'; '*.*'}, 'MultiSelect', 'on');
% 
% if ~iscell(file_name)
%     if file_name == 0
%         return
...
for n = 3 : file_num
    file = strcat(static_path, '/', file_list(n).name);
    img = imread(file);
    ...
    for i = 1 : step : height
        ...
        for j = 1 : step : width
            ...
            imwrite(region, [save_path sprintf('\\%03d-%06d.tif', n - 2, cropped_num_current_slice)]);
%             fprintf('x : %d  y : %d\n', idx_x, idx_y);
